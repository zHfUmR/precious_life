...
class WeatherCardVm extends _$WeatherCardVm {
  ...
  WeatherCardState build() => const WeatherCardState(
      weatherConfigState: WeatherCardConfigState(
  ...
  /// 天气配置
  Future<void> weatherKeyConfig() async {
      ...
      if (isConfigured) {
        ...
        if (isApiKeyValid) {
          ...
          state = state.copyWith(
              weatherConfigState:
                  const WeatherCardConfigState(loadingStatus: LoadingStatus.failure, errorMessage: 'API Key不可用'));
        }
      } else {
        state = state.copyWith(
            weatherConfigState:
                const WeatherCardConfigState(loadingStatus: LoadingStatus.failure, errorMessage: 'API Key未配置'));
      }
    } catch (e) {
      state = state.copyWith(
          weatherConfigState:
              WeatherCardConfigState(loadingStatus: LoadingStatus.failure, errorMessage: 'API Key配置失败: $e'));
    }
  }
  ...
  Future<void> weatherFollowed() async {
    ...
    try {
      ...
      final List<WeatherCardFollowedCityWeather> citiesWeather = [];
      for (final city in followedCities) {
        try {
          ...
          citiesWeather.add(WeatherCardFollowedCityWeather(city: city, weather: weatherResponse.now));
        } catch (e) {
          citiesWeather.add(WeatherCardFollowedCityWeather(city: city, errorMessage: e.toString()));
        }
      }
      state = state.copyWith(
          weatherFollowedState:
      ...
      state = state.copyWith(
          weatherFollowedState: WeatherCardFollowedState(
              loadingStatus: LoadingStatus.failure, errorMessage: '关注城市天气获取失败: ${e.toString()}'));
    }
  }
    ...
    try {
      ...
      for (final city in followedCities) {
        try {
          ...
          citiesWeather.add(WeatherCardFollowedCityWeather(
            city: city,
